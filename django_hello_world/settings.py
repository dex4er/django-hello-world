...
PROJECT_NAME = env.str('PROJECT_NAME', default=os.path.basename(os.path.dirname(os.path.abspath(__file__))))
...
] + EXTRA_BASE_MIDDLEWARE + BASE_MIDDLEWARE + LOCAL_MIDDLEWARE + [
...
SITE_DIR = env.str('SITE_DIR', default=os.path.realpath(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
    ...
    if db['ENGINE'].split('.')[-1] == 'mysql':
        ...
        if 'read_default_file' in db['OPTIONS']:
            if not os.path.isabs(db['OPTIONS']['read_default_file']):
                db['OPTIONS']['read_default_file'] = os.path.abspath(os.path.join(SITE_DIR, '.my.cnf'))
        db['TEST'] = {
            'NAME': 'test_' + db['NAME'],
