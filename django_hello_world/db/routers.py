...
class MultiDBRouter:
    ...
    def db_for_read(model, **_hints):
        return settings.DATABASE_APPS.get(model._meta.app_label, settings.DATABASE_APPS.get('*', 'default'))
    ...
    def allow_relation(obj1, obj2, **_hints):
        if obj1._meta.app_label in settings.PROJECT_APPS or obj2._meta.app_label in settings.PROJECT_APPS:
            return True
